...
void PaxosNode::ProcessClientRequest(const paxos::ClientRequest& request, SendClientRequestCallData* call_data) {
    ...
    if (role_ != Role::LEADER) {
        ...
        auto forward_call = new AsyncCall(peer_stubs_[leader_id_], client_cq_.get(), request, nullptr);
...
void PaxosNode::SendAcceptAckToLeader(const paxos::AcceptedEntry& entry) {
    ...
    // {
    //     std::lock_guard<std::mutex> lock(ongoing_accept_acks_mutex_);
    //     ongoing_accept_acks_[entry.seqnum()] = call;  
    // }
...
void PaxosNode::CommitEntry(const paxos::Ack ack) {
...

    {
        ...
        if (it == pending_entries_.end()) {
            ...
            // std::cout << "AcceptLog" << AcceptLogToString(accept_log_) << std::endl;
...
void PaxosNode::SequentiallyExecuteCommittedEntries() {
    ...
    while (true) {
        ...
        std::string two_pc_Status;
        {
            ...
            //          << " post break in sequentially seqnum to be executed is " << next_seq << " and seqnum from entry is " << it->second.seqnum() << std::endl;
            entry.mutable_ballot()->set_counter(it->second.ballot().counter());
            entry.mutable_ballot()->set_node_id(it->second.ballot().node_id());
            entry.set_seqnum(it->second.seqnum());
            entry.mutable_request()->CopyFrom(it->second.request());
...
void PaxosNode::SendPrepare(Ballot new_ballot) {
    ...
    for (auto& [peer_id, stub] : peer_stubs_) {
        ...
        //const paxos::PrepareRequest& const_request = request; // need a stable pointer for async call
...
void PaxosNode::OnElectionComplete() {
    ...
    for (auto& [seqnum, wal_entry] : wal_) {
        ...
        if (is_coordinator) {
            accounts_[from_account] = wal_entry.before_value; // revert deduction
            ...
            accounts_[to_account] = wal_entry.before_value; // revert addition
        }
    }
}
...
void PaxosNode::TransactionRetryLoop() {
    ...
    while (true) {
        ...
        // else {
        //     // Nothing to track — sleep forever or until woken by condition variable
